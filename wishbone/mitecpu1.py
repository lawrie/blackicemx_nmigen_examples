    ...
    def elaborate(self, platform):
        ...
        with m.FSM() as fsm:
            ...
            with m.State("READ/WRITE"):
                with m.If(code.ack):
                    m.next = "FETCH/EXECUTE"

        # Set requests for code and data on appropriate cycles
        # The wishbones buses currently used, do not support stall
        m.d.comb += [
            code.cyc.eq(fsm.ongoing("FETCH/EXECUTE") & dc.all()),
            code.stb.eq(code.cyc),
            data.cyc.eq(code.ack & fsm.ongoing("READ/WRITE")),
